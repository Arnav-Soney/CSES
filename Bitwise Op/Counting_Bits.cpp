...
ll countSetBits(ll n) {
    ...
    // x = largest power of 2 less or equal to n
    ll x = 0;
    while ((1LL << (x + 1)) <= n) x++;

    ll p = 1LL << x; // 2^x

    // Count of set bits in [1..p-1]
    ll bitsUpToPMinus1 = x * (p >> 1);

    // MSB set bits in [p..n]
    ll msbBits = n - p + 1;

    // Remaining bits after removing MSB contribution
    ll rest = countSetBits(n - p);

    return bitsUpToPMinus1 + msbBits + rest;
}
