...
ll solve(int n, ll sum1, ll sum2, vll& a){
    if(n <= 0){
        return abs(sum1 - sum2);
    }
    return min(solve(n-1, sum1 + a[n-1], sum2, a), solve(n-1, sum1, sum2 + a[n-1], a));
}
...
int32_t main(){
    ...
    cin >> n;
    vll a(n);
    for(int_fast64_t i=0 ; i<n ; i++) cin >> a[i];
    
    cout << solve(n, 0, 0, a) << endl;
