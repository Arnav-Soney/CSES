...
/*-----------------------------------------------------------------------------------------------------------------------*/
void topoSort(int node, vvll& adj, vbl& vis, stack<int>& st){
    vis[node] = true;
    for(int i: adj[node]){
        if(!vis[i]){
...
void dfs(int node, vvll& revAdj, vbl& vis, vi& SCC, int label){
    vis[node] = true;
    SCC[node] = label;
    for(int i: revAdj[node]){
        if(!vis[i]){
            dfs(i, revAdj, vis, SCC, label);
        }
    }
}
...
int32_t main(){
    ...
    while(!st.empty()){
        ...
        if(!vis[node]){
            dfs(node, revAdj, vis, SCC, label);
            label++;
        }
    }
    set<int> unique;
    rep(i, 0, n) unique.insert(SCC[i]);
